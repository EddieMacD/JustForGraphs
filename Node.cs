...
namespace JustForGraphs
{
    class Node<dataType>
    {
        ...
        {
            Neighbours.Add(new Edge<dataType>(this, neighbour, edgeWeight));
        }
        ...
        public List<Edge<dataType>> GetNeighbours()
        {
            return Neighbours;
        }
        ...
        public bool DepthFirstBool(List<dataType> nodes, Node<dataType> targetNode)
        {
            bool target = false;
            ...
            if (this != targetNode && !target )
            {
                ...
                foreach (Edge<dataType> edge in Neighbours)
                {
                    ...
                    if (!neighbour.GetSearchFlag() && !target)
                    {
                        target = neighbour.DepthFirstBool(nodes, targetNode);
                    }
                }
            }
            else
